...
impl Session {
    ...
    pub fn record_search(&self, query: &str) {
        ...
        let mut syms = self.symbols.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    fn record_dir(&self, path: &Path) {
        if let Some(dir) = path.parent() {
            ...
            let mut dirs = self.dir_hits.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub fn summary(&self) -> String {
        ...
        // Top symbols
        let syms = self.symbols.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
        if !syms.is_empty() {
            let mut sorted: Vec<_> = syms.iter().collect();
            sorted.sort_by(|a, b| b.1.cmp(a.1));
            let top: Vec<String> = sorted.iter().take(5).map(|(name, count)| {
                format!("{name} ({count})")
            }).collect();
        ...
        // Hot paths
        let dirs = self.dir_hits.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
        if !dirs.is_empty() {
            let mut sorted: Vec<_> = dirs.iter().collect();
            sorted.sort_by(|a, b| b.1.cmp(a.1));
            let top: Vec<String> = sorted.iter().take(5).map(|(dir, count)| {
                format!("{dir} ({count})")
            }).collect();
            let _ = write!(out, "\nHot paths: {}", top.join(", "));
        }
    ...
    pub fn reset(&self) {
        ...
        self.symbols.lock().unwrap_or_else(std::sync::PoisonError::into_inner).clear();
        self.dir_hits.lock().unwrap_or_else(std::sync::PoisonError::into_inner).clear();
    }
}
