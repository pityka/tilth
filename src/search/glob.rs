...
pub fn search(pattern: &str, scope: &Path) -> Result<GlobResult, TilthError> {
    ...
    walker.run(|| {
        ...
        Box::new(move |entry| {
            ...
            if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                extensions.lock().unwrap_or_else(std::sync::PoisonError::into_inner).insert(ext.to_string());
            }
            ...
            if matcher.is_match(name) || matcher.is_match(rel) {
                ...
                let mut locked = files.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    let files = files.into_inner().unwrap_or_else(std::sync::PoisonError::into_inner);
    let extensions = extensions.into_inner().unwrap_or_else(std::sync::PoisonError::into_inner);
